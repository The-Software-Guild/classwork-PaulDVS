...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public EmployeeList getAllEmployees() {
		return restTemplate.getForObject("http://localhost:8085/employees", EmployeeList.class);
	}
	...
	public boolean updateEmployeeSalById(int id, int newSal) {
		HttpHeaders header=new HttpHeaders();
		HttpEntity<Employee> entity=new HttpEntity<Employee>(header);
		Employee employee=restTemplate.exchange("http://localhost:8085/employees/"+id+"/"+newSal,HttpMethod.PUT, entity, Employee.class).getBody();
		if(employee!=null)
			return true;
		return false;
	}
